...
- 浏览器的主要组成部分是什么？
  ...
  - 1个浏览器主进程
    - 负责界面显示，交互，子进程管理，ti'gong
  - 1个GPU进程
    - 3D，Chrome UI绘制
  - 1个网络进程
    - 负责网络资源加载。之前是运行在浏览器进程里面的，后来才成为一个单独的进程。
  - 多个插件进程（一个插件对应一个进程）
    - 负责插件的与逆行，保证插件崩溃也不会对浏览器造成影响。
  - 多个渲染进程（一个页面对应一个进程）
...
- defer和async的区别？
  - 两者都是异步去加载外部JS文件，不会阻塞DOM解析
    - async时在外部JS加载完成后，浏览器空闲时，Load事件触发之前执行，标记为async的脚本并不保证按照指定它们的先后顺序执行，该属性对于内联脚本无用（即没有「src」属性的脚本） 
    - defer是在JS加载完成后，整个文档解析完成后，触发DOMContentLoaded事件前执行，同样的不会对内联脚本起作用
  - DOMContentLoaded 与 load 的区别 ?
    - DOMContentLoaded事件出发时：仅当DOM解析完成后，不包括样式表，图片等资源
    - onload事件触发时，页面上所有的DOM，样式表，脚本，图片等资源已经加载完毕
